...
each edge weight represents the dissimilarity between two nodes. Let $W'$
denote the matrix of dissimilarity weights of the graph. We then devised 
the following method to convert each dissimilarity edge weight 
$w'_{ij} \subset W'$ into a similarity edge weight $w_{ij}$:
\begin{align}
w_{ij} = \textrm{max}(0, 
	\argmaxA_{w'_{ij} \subset W} (w'_{ij} \neq \infty) - w'_{ij})
\end{align}

The conversion method consists of three possibilities:
\begin{itemize}
...
We then ran the spectral clustering on the similarity matrix $W$ in order to 
obtain $k$ clusters $K_{1}, K_{2}, ..., K_{k}$. Let $K_{c}$ denote the 
...
least one set of axioms that prove the conjecture and unsuccessful if
otherwise.
